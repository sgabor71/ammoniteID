...
(function () {
    ...
    // All transforms below verified by loading the real model in TF.js.
    // Keras 3 emits formats the tf.loadLayersModel parser rejects:
    //   1. "batch_shape" → "batch_input_shape" (InputLayer)
    //   2. DTypePolicy objects → plain string "float32"
    //   3. "silu" activation → "swish"
    //   4. inbound_nodes: {args,kwargs} objects → [[name,idx,idx,{}]] arrays
    //   5. input_layers/output_layers: flat → wrapped in outer array
    //   6. nested Functional models → flattened into parent (TF.js can't
    //      build nested containers loaded this way: "Graph disconnected")

    // Extract [layerName,nodeIdx,tensorIdx,{}] connections from K3 args
    function extractConnections(args) {
        const conns = [];
        (function walk(n) {
            if (!n) return;
            if (Array.isArray(n)) { n.forEach(walk); return; }
            if (typeof n === 'object' && n.class_name === '__keras_tensor__' && n.config && n.config.keras_history) {
                const kh = n.config.keras_history;
                conns.push([kh[0], kh[1], kh[2], {}]);
            }
        })(args);
        return conns;
    }

    function convertInboundNodes(ib) {
        if (!Array.isArray(ib) || ib.length === 0) return [];
        if (Array.isArray(ib[0])) return ib;          // already Keras 2 format
        const out = [];
        for (const node of ib) if (node && node.args) out.push(extractConnections(node.args));
        return out;
    }

    // Flatten nested Functional sub-models (e.g. efficientnetb0) into the parent.
    function flattenNestedFunctionals(topology) {
        const mc = topology.model_config || topology;
        const outerConfig = mc.config;
        if (!outerConfig || !outerConfig.layers) return topology;

        const getHistory = (ib) => {
            const refs = [];
            if (!Array.isArray(ib)) return refs;
            for (const node of ib) if (node && node.args) {
                (function w(n) {
                    if (!n) return;
                    if (Array.isArray(n)) { n.forEach(w); return; }
                    if (typeof n === 'object' && n.class_name === '__keras_tensor__' && n.config && n.config.keras_history)
                        refs.push(n.config.keras_history);
                })(node.args);
            }
            return refs;
        };

        const rewire = (layers, fromName, toName) => {
            for (const layer of layers) {
                if (!layer.inbound_nodes) continue;
                for (const node of layer.inbound_nodes) {
                    if (node && node.args) {
                        (function w(n) {
                            ...
                            if (Array.isArray(n)) { n.forEach(w); return; }
                            if (typeof n === 'object' && n.class_name === '__keras_tensor__' && n.config && n.config.keras_history)
                                if (n.config.keras_history[0] === fromName) n.config.keras_history[0] = toName;
                        })(node.args);
                    }
                }
            }
        ...
        const newLayers = [];
        const funcMap = {};   // nested layer name → its inner output layer name
        for (const layer of outerConfig.layers) {
            if (layer.class_name === 'Functional' && layer.config && layer.config.layers) {
                const inner = layer.config;
                const calledOn = getHistory(layer.inbound_nodes);
                const outerSource = calledOn.length ? calledOn[0][0] : null;
                const innerInputName  = Array.isArray(inner.input_layers[0])  ? inner.input_layers[0][0]  : inner.input_layers[0];
                const innerOutputName = Array.isArray(inner.output_layers[0]) ? inner.output_layers[0][0] : inner.output_layers[0];

                const innerReal = inner.layers.filter(il => !(il.class_name === 'InputLayer' && il.name === innerInputName));
                // Rewire inner layers that referenced the inner input → outer source
                if (outerSource) rewire(innerReal, innerInputName, outerSource);
                newLayers.push(...innerReal);
                funcMap[layer.name] = innerOutputName;
            } else {
                newLayers.push(layer);
            }
        }
        // Downstream layers that referenced the nested model → its inner output
        for (const [nestedName, innerOut] of Object.entries(funcMap)) {
            rewire(newLayers, nestedName, innerOut);
        }
        outerConfig.layers = newLayers;
        return topology;
    }
    ...
    function patchKeras3Topology(obj) {
        ...
        if (obj.activation === 'silu') obj.activation = 'swish';

        // Wrap flat input_layers / output_layers
        if (obj.input_layers && Array.isArray(obj.input_layers) && typeof obj.input_layers[0] === 'string')
            obj.input_layers = [obj.input_layers];
        if (obj.output_layers && Array.isArray(obj.output_layers) && typeof obj.output_layers[0] === 'string')
            obj.output_layers = [obj.output_layers];

        // Convert K3 inbound_nodes objects → K2 arrays
        if (obj.inbound_nodes && Array.isArray(obj.inbound_nodes) &&
            obj.inbound_nodes.length > 0 && !Array.isArray(obj.inbound_nodes[0])) {
            obj.inbound_nodes = convertInboundNodes(obj.inbound_nodes);
        }

        for (const key of Object.keys(obj)) {
            if (key !== 'inbound_nodes' && typeof obj[key] === 'object' && obj[key] !== null) {
    ...
    // Collect DepthwiseConv2D layer names so we can rename their weights
    // (Keras 3 names the kernel "kernel"; TF.js expects "depthwise_kernel")
    function collectDepthwiseNames(obj, set) {
        if (!obj || typeof obj !== 'object') return;
        if (Array.isArray(obj)) { obj.forEach(o => collectDepthwiseNames(o, set)); return; }
        if (obj.class_name === 'DepthwiseConv2D' && obj.config && obj.config.name) set.add(obj.config.name);
        for (const k of Object.keys(obj)) if (typeof obj[k] === 'object' && obj[k] !== null) collectDepthwiseNames(obj[k], set);
    }
    ...
    function registerCustomLayers() {
        ...
        class NormalizationLayer extends tf.layers.Layer {
            ...
            call(inputs) {
                ...
                return tf.tidy(() => tf.div(tf.sub(x, mean), tf.sqrt(tf.add(variance, this.epsilon))));
            }
    ...
    function buildIOHandler(modelJson, shardBuffers) {
        return {
            load: async () => {
                ...
                // Concatenate shard buffers in manifest path order
                ...
                // Patch + flatten the Keras 3 topology
                const topology = JSON.parse(JSON.stringify(modelJson.modelTopology));
                flattenNestedFunctionals(topology);
                patchKeras3Topology(topology);

                // Rename DepthwiseConv2D kernel weights → depthwise_kernel
                const dwNames = new Set();
                collectDepthwiseNames(topology, dwNames);
                const weightSpecs = weightsManifest.flatMap(g => g.weights).map(s => {
                    for (const dw of dwNames) {
                        if (s.name === dw + '/kernel') return { ...s, name: dw + '/depthwise_kernel' };
                    }
                    return s;
                });
    ...
    // Model has built-in Rescaling(1/255) + Normalization layers,
    // so pass raw 0-255 pixel values.
