...
def generate_confidence_message(
    ...
    Returns dict with:
    - family_label: "HIGH ✅", "MODERATE ⚠️", or "LOW ❌"
    - genus_label: same
    ...
    feedback_message = ""
    feedback_style = "info"  # 'info' (blue) or 'warning' (yellow/orange)
    
    # Generate feedback based on scenario and confidence
    if scenario == 'non_ammonite':
        if top_family_score >= 75:
            # High confidence non-ammonite
            ...
            feedback_style = "info"
        elif top_family_score >= 55:
            # Moderate confidence non-ammonite
            feedback_message = "💡 If you believe this is actually an ammonite, try retaking with the spiral/coiling pattern clearly visible"
            feedback_style = "info"
        else:
            # Low confidence non-ammonite
            feedback_message = "⚠️ Unable to confidently identify this specimen. 💡 If you believe this is actually an ammonite, for better results, try: retake and fill 80%+ of frame, better lighting, sharper focus"
            feedback_style = "warning"
    
    elif scenario == 'uncertain':
        feedback_message = "⚠️ Low confidence - for a more reliable result, try: closer photo, better lighting, or fill 80%+ of frame"
        feedback_style = "warning"
    
    elif scenario in ['low_confidence', 'moderate_confidence', 'possible']:
        # Family confidence check
        if top_family_score < 55:
            feedback_message = "⚠️ Low confidence - for a more reliable result, try: closer photo, better lighting, or fill 80%+ of frame"
            feedback_style = "warning"
        elif top_family_score < 75:
            feedback_message = "💡 This result is likely correct. For even better accuracy, try filling 80%+ of frame or rotating 30-90°"
            feedback_style = "info"
        else:
            # Family high, genus low/moderate
            if top_genus_score and top_genus_score < 55:
                feedback_message = "⚠️ Genus unclear - a closer photo showing ribs/sutures may help refine the identification"
                feedback_style = "warning"
            elif top_genus_score and top_genus_score < 75:
                feedback_message = "💡 Family identification is confident. For better genus accuracy, try adding another photo rotated 30-90°"
                feedback_style = "info"
    
    # High confidence scenarios - check if any is moderate
    elif scenario in ['likely']:
        if top_family_score < 75 or (top_genus_score and top_genus_score < 75):
            if top_genus_score and top_genus_score < 75:
...
def build_result(
    ...
    confidence_data = generate_confidence_message(
        ...
        top_genus_score=top_genus_score,  # Already rounded in backend
        scenario=scenario
    )
